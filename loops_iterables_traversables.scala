...
// A few subclasses of `Iterable` override this standard implementation of `foreach` in `Iterable` to  provide a more efficient implementation
...
// Iterator has two important methods: `hasNext`, which answers whether the iterator has another element available, 
...
xs1.iterator.sameElements(ys1) should be(true)  // `set` specific behavior 
